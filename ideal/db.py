...
class Database:
    ...
    def clear_alerts(self, consumer: str = "all") -> int:
        normalized = consumer.strip().lower()
        with self.connection() as conn:
            if normalized == "all":
                cursor = conn.execute("DELETE FROM alerts")
            elif normalized in {"digest", "watchlist"}:
                cursor = conn.execute(
                    "DELETE FROM alerts WHERE consumer=?", (normalized,)
                )
            else:
                raise ValueError("consumer 必须是 digest、watchlist 或 all")
            return max(0, int(cursor.rowcount))

    def import_legacy_observations(
        self, rows: Iterable[tuple[str, str, Decimal, str, str]]
    ) -> int:
        """导入旧库价格历史；按时间顺序重放，避免污染状态机。"""
        count = 0
        for app_id, region, price, currency, captured_at in rows:
            ...
            app = OfficialApp(
                ...
                price=price,
                currency=currency,
                formatted_price=decimal_text(price) or "",
                url=f"https://apps.apple.com/{region}/app/id{app_id}",
            )
            self.observe(app, run_id, "legacy")
            with self.connection() as conn:
                conn.execute(
                    """
                    UPDATE price_observations SET captured_at=?
                    WHERE run_id=? AND app_id=? AND region=?
                    """,
                    (captured_at, run_id, app_id, region),
                )
            count += 1
        return count
